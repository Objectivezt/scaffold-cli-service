...
const {
  ...
  ScriptStringAfter,
  ScriptStringBefore,
  INCLUDE_REGEXP_STRING,
} = require('../config/libCdnConfig');
const { theme, alias, mode } = require('../config/exportWebpackConfig');
...
let NODE_MODULE_REGEXP = null;
if (INCLUDE_REGEXP_STRING) {
  NODE_MODULE_REGEXP = INCLUDE_REGEXP_STRING.replace(/(?<=\\)(\/|\\)/g, `${path.sep}`);
}
...
const EXCLUDE_REGEXP_STRING = new RegExp(
  `node_modules\\${path.sep}(?!@scaffold|antd|${NODE_MODULE_REGEXP})`,
);

const modeSingle = [
  {
    test: /\.css$/,
    exclude: EXCLUDE_REGEXP_STRING,
    use: [
  ...
  {
    test: /\.less$/,
    exclude: [/node_modules/, EXCLUDE_REGEXP_STRING],
    use: [
      MiniCssExtractPlugin.loader,
      {
        loader: require.resolve('css-loader'),
        options: {
          ...
          modules: {
            localIdentName: IS_DEV
              ? '[path][name]__[local]--[hash:base64:5]---dev'
              : '[hash:base64]---prd',
          },
        },
      },
...
const modeDouble = [
  {
    test: CSS_LESS_REGEXP,
    exclude: [EXCLUDE_REGEXP_STRING, CSS_LESS_MODULE_REGEXP],
    use: cssLoaderConfig(false, 'less'),
  },
  {
    test: CSS_LESS_MODULE_REGEXP,
    exclude: EXCLUDE_REGEXP_STRING,
    user: cssLoaderConfig(true, 'less'),
  },
];

const cssMode = mode ? modeDouble : modeSingle;
...
module.exports = {
  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        exclude: [EXCLUDE_REGEXP_STRING, SPRITE_SVG_REGEXP],
        use: [
          {
            loader: 'babel-loader',
          },
  ...
  plugins: [
    ...
    new CopyWebpackPlugin([
      ...
      {
        from: path.resolve(__dirname, `../../dll/vendor${IS_DEV ? '.dev' : ''}.dll.js`),
        to: path.join(PROJECT_PATH, './dist/dll/'),
      },
    ]),
  ],
  resolve: {
